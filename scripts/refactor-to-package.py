...
"""
Things move around a not during dev...so this can help
"""

OLDPACK = "package_to_change"
NEWPACK = "new_pack_name"
...
RE_IMPORT_FROM = re.compile(rf"\bfrom {OLDPACK}(\b|\.)(.*)")
RE_IMPORT = re.compile(rf"\bimport {OLDPACK}(\b|\.)(.*)")


def replace_imports(text: str) -> tuple[str, int]:
  count = 0
  new = text
  new2, n1 = RE_IMPORT_FROM.subn(lambda m: f"from {NEWPACK}{m.group(1)}{m.group(2)}",
                                 new)
  new = new2
  new2, n2 = RE_IMPORT.subn(lambda m: f"import {NEWPACK}{m.group(1)}{m.group(2)}", new)
  count = n1 + n2
  return new2, count
...
def patch_pyproject(pyproj_path: Path) -> tuple[bool, dict]:
  ...
  def _name_sub(m):
    nonlocal changed
    old = m.group(1)
    new = old.replace(OLDPACK, NEWPACK)
    if new != old:
      changed = True
  ...
  def _ep_sub(m):
    ...
    new_val = val.replace(f"{OLDPACK}.", f"{NEWPACK}.")
    if new_val != val:
      changed = True
